...
| `LOG_CLASS_VEL_FUSION` | `0x20` | `state_estimation.c` | Optical-flow body-velocity diagnostics (6 floats / 24 B): DOWN camera measured `v_body_xy` (m/s), ESKF-predicted `v_body_xy`, clarity and range. Velocity fused via `fusion6_update_velocity_xy_body`. UP camera is not fused (no range finder). |
...
| `optflow_velocity_view.py` | Optical-flow body-velocity viewer: DOWN camera measured vx/vy time-series overlaid on the ESKF-predicted body velocity, plus clarity and range strips. Uses `LOG_CLASS_VEL_FUSION`. |
