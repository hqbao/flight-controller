...
| `LOG_CLASS_VEL_FUSION` | `0x20` | `state_estimation.c` | Optical-flow body-velocity diagnostics (6 floats / 24 B): DOWN camera measured `v_body_xy` (m/s), ESKF-predicted `v_body_xy`, clarity and range. Velocity fused via `fusion6_update_velocity_xy_body` with a static per-axis variance from `cfg.R_vel_xy_body` (default σ=0.10 m/s). UP camera is not fused (no range finder). When the rangefinder reports below the optflow gate (z < 200 mm), a **ZUPT** (zero-velocity update) is fused at the same per-axis variance instead of leaving the estimator open-loop. `v_meas` is set to NaN whenever the frame is rejected so the viewer leaves a visible gap rather than holding a stale value. |
| `LOG_CLASS_BARO_FUSION` | `0x21` | `state_estimation.c` | Barometric altitude diagnostics (5 floats / 20 B): `z_meas` (NED, +down), ESKF `p.z`, ESKF `v.z`, innovation, `baro_ok` flag. Altitude fused via 1-D scalar `fusion6_update_baro_z` with `cfg.R_baro` (default σ=0.5 m). The first baro frame after `INIT_DONE` **snaps** `p.z = z_meas` and `v.z = 0` to escape the ±50 m sanity gate that would otherwise lock baro out after the accel-only drift between `INIT_DONE` (~0.25 s after boot) and the first baro frame (~4 s after boot, due to the air_pressure 100-sample warmup). No bias state — `air_pressure.c` already zeros the offset at startup. |
...
| `optflow_velocity_view.py` | Optical-flow body-velocity viewer: DOWN camera measured vx/vy time-series overlaid on the ESKF-predicted body velocity, plus clarity and range strips (Y axes auto-scale). Right pane is a 2D top-down body-frame view that integrates `v_meas` into a path trail with current `v_meas` / `v_pred` arrows; recenters when the trail leaves the ±2 m box. Includes a Clear button that wipes the chart history without resetting the FC. Uses `LOG_CLASS_VEL_FUSION`. |
| `baro_fusion_view.py` | Barometric altitude fusion viewer: stacked strips for altitude (`z_meas` vs ESKF `p.z`, displayed +up so visual matches intuition), `v.z` (NED, +down), and innovation `z_meas − p.z` (autoscaled from data). Status row shows raw NED `z_meas` / `p.z` / `v.z` / innov, with a `[GATED]` tag if the innovation exceeds the FC's ±50 m sanity gate. `BARO_OK` lamp lights when `FUSION6_HF_BARO_OK` is set. Uses `LOG_CLASS_BARO_FUSION`. |
