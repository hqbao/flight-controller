...
| `LOG_CLASS_VEL_FUSION` | `0x20` | — | *(Removed — subsumed by `LOG_CLASS_OPTFLOW_DEROT` (`0x22`), which carries `v_meas` / `v_pred` / `range` / `clarity` plus the raw + derotated flow rates.)* |
...
| `LOG_CLASS_OPTFLOW_DEROT` | `0x22` | `state_estimation.c` | Optical-flow pipeline diagnostic (10 floats / 40 B @ 25 Hz): `flow_raw_xy` and `flow_derot_xy` (rad/s, body), `v_meas_xy` and `v_pred_xy` (m/s, body), `range` (m), `clarity`. Lets the operator see every stage from raw camera flow through gyro derotation through range scaling to the ESKF prediction on a single chart. Velocity fused via `fusion6_update_velocity_xy_body` with a static per-axis variance from `cfg.R_vel_xy_body` (default σ=0.10 m/s). UP camera is not fused (no range finder). When the rangefinder reports below the optflow gate (z < 200 mm), a **ZUPT** (zero-velocity update) is fused at the same per-axis variance. `flow_*` fields are NaN until a frame with a valid `dt_us` arrives; `v_meas` is NaN whenever the frame is rejected so the viewer leaves a visible gap. |
...
| `optflow_derot_view.py` | Optical-flow pipeline viewer: stacked body-X / body-Y strips overlay raw flow, derotated flow (both rad/s, scaled to m/s using the latest range), `v_meas` (= derot · range, what gets fused) and ESKF-predicted `v_pred` on a shared m/s axis. Third strip shows clarity. Use during bring-up to spot bad derotation (raw·range diverges from derot·range under pure rotation while v_meas stays near zero) or fusion mismatches (v_meas and v_pred should track during translation). Uses `LOG_CLASS_OPTFLOW_DEROT`. |
