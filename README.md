...
│   ├── eskf_view.py               #   ESKF matrix viewer — heatmap of P / F / K / H
...
| `LOG_CLASS_ESKF_P` / `_F` / `_K` / `_H` | `0x24`–`0x27` | `state_estimation.c` | ESKF matrix introspection — read-only snapshots of the four cached `fusion6` matrices: covariance `P` (15×15), last predict-step Jacobian `F` (15×15), last update Kalman gain `K` (15×m) and measurement Jacobian `H` (m×15). Streamed paged: one row per dblink frame (12 B header + 4·cols B), full matrix per scheduler tick @ 1 Hz. K/H carry the last update's `update_type` tag (accel / mag / vel_xy_body / baro / gps_pos / gps_vel) so the viewer can label which sensor produced the gain. Caches are populated by `fusion6_predict` / `fusion6_update_*` and read via `fusion6_get_matrix()` — they do not feed back into the filter math. Consumed by `tools/eskf_view.py`. |
...
| `optflow_view.py` | Optical-flow viewer (4 stacked strips): body-X velocity (m/s), body-Y velocity (m/s) — each overlaying camera measurement (`v_meas`, green) vs ESKF prediction (`v_pred`, blue); range (m, orange); clarity (purple). Use during bring-up to spot fusion mismatches (v_meas and v_pred should track during translation) and to verify the rangefinder is returning sensible values across the working envelope. Uses `LOG_CLASS_OPTFLOW`. |
| `baro_fusion_view.py` | Barometric altitude fusion viewer: stacked strips for altitude (`z_meas` vs ESKF `p.z`, displayed +up so visual matches intuition) and `v.z` (NED, +down). Status row shows raw NED `z_meas` / `p.z` / `v.z`. `BARO_OK` lamp lights when `FUSION6_HF_BARO_OK` is set. Uses `LOG_CLASS_BARO_FUSION`. |
...
| `eskf_view.py` | ESKF matrix viewer: live diverging-colormap heatmap of one of the four cached `fusion6` matrices (`P`, `F`, `K`, `H`) with linear/log-scale toggle, side panel showing matrix shape / last `update_type` / min / max / Frobenius norm / seq, and a rolling `‖M‖_F` strip. Buttons select which matrix streams; only one is active at a time (FC's pubsub log-class gating). Uses `LOG_CLASS_ESKF_{P,F,K,H}`. |
